...
export class ThemeService {
  ...
  isDarkMode$: Observable<boolean> = this.isDarkModeSubject
    .asObservable()
    .pipe(tap((isDarkMode: boolean) => this.applyDarkMode(isDarkMode)));

  private presetSubject: BehaviorSubject<PrimePreset> = new BehaviorSubject<PrimePreset>(
    this.initPreset(),
  );

  preset$: Observable<PrimePreset> = this.presetSubject
    .asObservable()
    .pipe(tap((preset: PrimePreset) => this.setPreset(preset)));
